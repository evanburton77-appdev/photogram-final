class FollowRequestsController < ApplicationController
  ...
  def create
    ...
    if User.where({ :id => the_follow_request.recipient_id }).first.private == true
    ...
    if the_follow_request.valid?
      ...
      redirect_to("/users/" + User.where({ :id => the_follow_request.recipient_id }).first.username, { :notice => "Follow request created successfully." })
    else
